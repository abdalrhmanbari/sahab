...
import { carSale, houseSale, villasRent, villasSale } from '../../Context/context'
  ...
  return (
    ...
    <div className=' flex flex-row-reverse  gap-4 w-full '>
      ...
      <div className=' w-[80%] flex flex-col gap-4'>
        {houseSale?.map((data) =>(
          <Cart data={data} key={data.id} link={`/real-estate/house-sale/${data.id}`} />
        ))}
        <SponsoredAd/>
        {villasSale?.map((data) =>(
          <Cart data={data} key={data.id} link={`/real-estate/villas-sale/${data.id}`} />
        ))}
        <SponsoredAd/>
        {villasRent?.map((data) =>(
