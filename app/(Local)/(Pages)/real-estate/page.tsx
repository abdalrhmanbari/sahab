...
export default function RealEstate() {
  return (
    <div className=' flex flex-col items-end gap-4 text-end 2xl:px-40 sm:px-16 px-4'>
        ...
        <h1 className=' text-[#222222]'>عقارات </h1>
      </span>
      <h1 className=' sm:text-2xl text-lg font-semibold mb-2'>عقارات للبيع والإيجار في سوريا</h1>
    ...
    <div className=' bg-blue-100 flex flex-col gap-2 w-[10rem] md:w-[12rem] pe-10  py-8   h-fit  ' dir='ltr' >
        ...
        <Link href={"/real-estate/houses-rent"} className=' text-[0.8rem] text-[#23262AA3]  hover:text-[#222222]'>منزل للإيجار</Link>
      ...
      <div className=' sm:w-[80%] w-full flex flex-col gap-4'>
        {houseSale?.map((data) =>(
          <Cart data={data} key={data.id} link={`/real-estate/houses-sale/${data.id}`}  cars={false} realEstate={true}/>
        ))}
        <SponsoredAd/>
        {villasSale?.map((data) =>(
        ...
        <SponsoredAd/>
        {houseRent?.map((data) =>(
          <Cart data={data} key={data.id} link={`/real-estate/houses-rent/${data.id}`} cars={false} realEstate={true}/>
        ))}
        <SponsoredAd/>
        {villasRent?.map((data) =>(
